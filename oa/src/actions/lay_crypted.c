...
int lay_crypted(OA_Context *ctx) {
    ...
    if (!cJSON_IsString(pathLiveFs)) {
        LOG_ERR("lay_crypted: pathLiveFs missing");
        return 1;
    }
    ...
    if (stat(squash_path, &st) != 0) {
        LOG_ERR("lay_crypted: %s non trovato. Lo step lay_squash ha fallito?", squash_path);
        return 1;
    }
    ...
    if (system(cmd) != 0) {
        LOG_ERR("lay_crypted: Creazione root.img fallita");
        return 1;
    }
    ...
    if (system(cmd) != 0) {
        LOG_ERR("lay_crypted: luksFormat fallito");
        return 1;
    }
    ...
    if (system(cmd) != 0) {
        LOG_ERR("lay_crypted: luksOpen fallito");
        return 1;
    }
    ...
    res = system(cmd);
    if (res != 0) {
        LOG_ERR("lay_crypted: mkfs.ext4 fallito. Eseguo cleanup...");
