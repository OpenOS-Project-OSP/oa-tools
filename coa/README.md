...
**coa** is the intelligent orchestrator written in Go, designed to be the "Mind" behind the GNU/Linux system remastering process. It acts as a high-level interface for the native **oa** (Output Artisan) engine, handling system analysis, flight plan generation, and abstracting distribution-specific differences.

## 🚀 Key Features

* **Distro-Agnostic Intelligence**: Automatically recognizes Debian, Arch, Fedora, and their derivatives.
* **Initramfs Abstraction**: Delegates initrd generation to the engine via dynamic templates, supporting `mkinitcpio`, `dracut`, and `mkinitramfs`.
