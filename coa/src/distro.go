...
func resolveDerivative(distroID string, codenameID string) (bool, *Distro) {
	...
	if err != nil {
		fmt.Println("\033[1;33m[cova]\033[0m Attenzione: file derivatives.yaml non trovato.")
		return false, nil
	}

	var mappings []DerivativeMapping
	if err := yaml.Unmarshal(yamlData, &mappings); err != nil {
		fmt.Printf("\033[1;31m[cova]\033[0m Errore nel parsing di derivatives.yaml: %v\n", err)
		return false, nil
	}

	for _, mapping := range mappings {
		for _, deriv := range mapping.Derivatives {
			// Match case-insensitive per ID o Codename
...
func NewDistro() *Distro {
	...
	// 2. Se non è una madre pura, cerchiamo tra le derivate
	found, derivativeDistro := resolveDerivative(rawID, rawCodename)
	if found {
		return derivativeDistro
	}

	// 3. Fallback d'emergenza se non riconosciuta
	fmt.Printf("\033[1;31m[cova]\033[0m Distro sconosciuta (%s/%s). Aggiungila a derivatives.yaml!\n", rawID, rawCodename)
	os.Exit(1)
	return nil
}
